...
const SUNO_SONG_STRUCTURE = {
  sections: [
    { tag: "[Verse1]", prompt: "Generate the first verse" },
    { tag: "[Chorus1]", prompt: "Create a catchy, memorable chorus" },
    { tag: "[Verse2]", prompt: "Write a second verse with progression" },
    { tag: "[Bridge]", prompt: "Compose a bridge that adds emotional depth" },
    {
      tag: "[Verse3]",
      prompt: "Craft a final verse that brings the song to a climax",
    },
    {
      tag: "[Chorus2]",
      prompt: "End with the chorus to reinforce the main message",
    },
  ],
};
...
export default function Home() {
  ...
  useEffect(() => {
    const allEnvVars = {
      NEXT_PUBLIC_OPENROUTER_API_KEY: (window as any).ENV
        ?.NEXT_PUBLIC_OPENROUTER_API_KEY
        ? "SET"
        : "NOT SET",
      NEXT_PUBLIC_OPENAI_API_KEY: (window as any).ENV
    ...
    const providerDebug = providers.map((provider) => ({
      ...
      apiKeyStatus: (window as any).ENV?.[provider.apiKeyEnvVar]
        ? "SET"
        : "NOT SET",
    }));
    ...
    try {
      ...
      for (const section of SUNO_SONG_STRUCTURE.sections) {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an elite hip-hop lyricist creating lyrics for Suno AI. Use strict Suno vocal tag formatting. Always enclose lyrics in the appropriate section tag. Use parentheses () for callback/adlib vocals. Maintain complex rhyme schemes and authentic street wisdom.",
            },
            {
              role: "user",
        ...
        if (selectedProvider.name === "OpenRouter") {
          ...
          // Add debug logging for OpenRouter request
          console.log("OpenRouter Request Headers:", headers);
          console.log("OpenRouter Request URL:", selectedProvider.apiUrl);
        }
              ...
              <div className="space-y-2">
                <button
                  ...
                  onClick={(e) => {
                    ...
                    e.stopPropagation();
                    try {
                      console.log("Button clicked: OpenRouter");
                      ...
                      if (provider) {
                        ...
                        fetchModels(provider).catch((error) => {
                          console.error("Error fetching models:", error);
                        });
                      } else {
                        console.error(
                          "OpenRouter provider not found in providers list"
                        );
                      }
                    } catch (error) {
