    ...
    try {
      ...
      const requestBody = {
        ...
        messages: [
          {
            role: "system",
            content: `You are a professional lyricist and poet. Generate intricate, creative, and original song lyrics based on the provided prompt. The lyrics must follow this specific format:

1. Use section tags: [Intro], [Verse1], [Verse2], [Pre-Chorus], [Chorus], [Verse3], [Verse4], [Pre-Chorus], [Chorus], [Interlude], [Bridge], [Verse5], [Pre-Chorus], [Chorus], [Outro], [END]
2. [Verse] sections are 16 bars long
3. [Chorus] is 8 or 16 bars long
4. [Pre-Chorus] is 4 bars long
5. [Bridge] is 8 bars long
6. [Intro] should be 4-6 lines in a spoken/narrative style
7. Use (parentheses) for call-and-response/adlib vocals
8. Replace vowels with * for explicit words
9. Include complex rhyme schemes (multi-syllable, internal rhymes, slant rhymes)
10. Use poetic devices (alliteration, assonance, metaphors)
11. Maintain thematic cohesion and emotional progression`,
          },
          {
            role: "user",
            content: `Generate complete song lyrics about "${theme}". Follow the structure and formatting rules defined above. Make it emotionally impactful with vivid imagery and advanced rhyme techniques.`,
          },
        ],
        temperature: 0.7,
      };
                ...
                {availableModels.map((model) => {
                  ...
                  if (selectedProvider?.name === "OpenRouter") {
                    ...
                    displayName = displayName
                      .split(/[(.]/)[0] // Remove everything after period or opening parenthesis
                      .replace(
                        /\s*\b(?:official|instruct|fine-tuned|version|of)\b.*/i,
                        ""
                      ) // Remove common descriptive phrases
                      .replace(/\s+v\d+.*$/i, "") // Remove version numbers
                      .replace(/\s*launched.*$/i, "") // Remove "launched with..." descriptions
                      .replace(/\bThis\b.*$/i, "") // Remove sentences starting with "This"
