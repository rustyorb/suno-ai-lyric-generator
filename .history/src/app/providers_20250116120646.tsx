...
if (!Array.isArray(models)) {
  throw new Error('Model list is not an array');
      ...
      const processedModels = models.map((model: ApiModel) => {
        let displayName = model.name || model.id;
        
        // For OpenRouter, extract a cleaner name
        if (provider.name === "OpenRouter" && model.description) {
          ...
          if (desc.includes("is")) {
            ...
            displayName = words[0].toLowerCase() === "the"
      ...
      const filteredModels =
        provider.name === "OpenRouter"
          ? models
          : models.filter((model: { id: string; name: string }) => {
              const id = model.id.toLowerCase();
              return (
                // OpenAI models
                id.includes("gpt-4") ||
                id.includes("gpt-3.5-turbo") ||
                // Include all other models that might be chat-capable
                id.includes("chat") ||
