  ...
  const fetchModels = async (provider: AIProvider) => {
    ...
    try {
      ...
      let models = modelList.map((model: ApiModel) => {
        // For OpenRouter, extract model name from description if available
        const name = provider.name === "OpenRouter" && model.description
          ? model.description.split(" ")[0] // Get the first part of description as name
          : model.name || model.id;
