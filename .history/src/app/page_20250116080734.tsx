  ...
  const generateLyrics = async () => {
    ...
    try {
      ...
      for (const section of SUNO_SONG_STRUCTURE.sections) {
        ...
        const lyricContent = response.data.choices[0].message.content.trim();
        // Don't add the section tag if it's already in the content
        if (!lyricContent.includes(section.tag)) {
          fullLyrics += `\n${section.tag}\n${lyricContent}\n`;
        } else {
          fullLyrics += `\n${lyricContent}\n`;
        }
