...
export default function ProviderManager({ 
  ...
  const handleAddProvider = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (name && apiUrl) {
      addProvider({
        name,
        apiUrl,
        apiKeyEnvVar: `NEXT_PUBLIC_${name.toUpperCase().replace(/\s+/g, '_')}_API_KEY`,
        authHeaderPrefix: authHeaderPrefix || '',
        modelsEndpoint: modelsEndpoint || `${apiUrl.replace('/chat/completions', '')}/models`,
        modelPathInResponse: '',
        modelNamePath: 'id',
        modelIdPath: 'id'
      })
  ...
  const handleEditProvider = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Remove old provider
    removeProvider(editingProvider.name)
  ...
  const ProviderForm = ({ provider, onSubmit }: { provider?: AIProvider, onSubmit: (e: React.FormEvent<HTMLFormElement>) => void }) => (
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg">
      ...
      {editingProvider && (
        <>
          <h4 className="text-lg font-medium text-purple-400 mb-3">Edit Provider</h4>
          <ProviderForm provider={editingProvider} onSubmit={handleEditProvider} />
          <button
